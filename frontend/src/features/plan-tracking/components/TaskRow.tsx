...
function canFullEdit(task: TaskListItem, scope: "main" | "sub", userRoles: string[], planStatus: string | null) {
...
function canProgressEdit(task: TaskListItem, scope: "main" | "sub", userRoles: string[], planStatus: string | null) {
...
export function TaskRow({
  ...
  const fullEdit = canFullEdit(task, scope, userRoles, planStatus);
  const progressEdit = canProgressEdit(task, scope, userRoles, planStatus);
    ...
    <tr className={rowClass(task)}>
      ...
      <td className="c-workstatus">
        <select
          ...
          onBlur={() => handleBlurWithDeferredSave("workStatus")}
          onChange={(e) => setDraft((prev) => ({ ...prev, workStatus: e.target.value as WorkStatus }))}
          onFocus={() => setActiveField("workStatus")}
      ...
      <td className="c-person">
        <div className="ph-wrap">
          {submitHistory ? (
            <>
              <div className="ph-name">{submitHistory.actorUserName || "Văn thư"}</div>
              <div className="ph-meta">{formatVnDateTime(submitHistory.createdAt)}</div>
              {submitHistory.comment && (
                <div className="ph-comment">"{submitHistory.comment}"</div>
              )}
            </>
          ) : task.assigneeName ? (
            <>
            ...
            <span className="ph-empty">(Chưa có)</span>
          )}
