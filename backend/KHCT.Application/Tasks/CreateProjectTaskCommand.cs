...
public class CreateProjectTaskHandler : IRequestHandler<CreateProjectTaskCommand, TaskDetailDto>
{
    private readonly IApplicationDbContext _db;
    private readonly ICurrentUser _currentUser;

    public CreateProjectTaskHandler(IApplicationDbContext db, ICurrentUser currentUser)
    {
        _db = db;
        _currentUser = currentUser;
    }

    public async Task<TaskDetailDto> Handle(CreateProjectTaskCommand request, CancellationToken ct)
    {
        var project = await _db.Projects.AsNoTracking().FirstOrDefaultAsync(x => x.Id == request.ProjectId, ct)
            ?? throw new KeyNotFoundException("Project not found.");

        // Section 4.3.2: Leader/Admin can create tasks
        var isAuthorized = project.LeaderId == _currentUser.UserId ||
                           project.SubLeaderId == _currentUser.UserId ||
                           PlanSupport.HasRole(_currentUser, PlanSupport.RoleAdmin);

        if (!isAuthorized)
        {
            throw new ForbiddenException("forbidden_task_creation", "Only Project Leader, SubLeader or Admin can create project tasks.");
        }
        ...
        if (request.ParentTaskId.HasValue)
        {
            ...
            if (parent.Category != TaskCategory.ProjectTask || parent.ProjectId != request.ProjectId)
            {
                throw new DomainException("parent_task_invalid", "Parent task must belong to the same project.");
            }
        }
        ...
        var task = new TaskEntity
        {
            ...
            Category = TaskCategory.ProjectTask,
